...
Current task: Implementing new features`;
      ...
      if (msg && "summary" in msg) {
        ...
        expect(msg.retainedKnowledge).toEqual(["IPC protocol", "tmux integration", "logging system"]);
...
Status: idle`;
    ...
    it("returns null for non-protocol text", () => {
      const msg = parseProtocolMessage("Hello, this is just regular text");
      expect(msg).toBeNull();
    });

    it("returns null for malformed protocol message", () => {
      const msg = parseProtocolMessage("[PROTOCOL:INVALID missing fields");
      expect(msg).toBeNull();
    });
  });
  ...
  describe("isProtocolMessage()", () => {
    it("returns true for protocol messages", () => {
      expect(isProtocolMessage("[PROTOCOL:HEARTBEAT] from x at y")).toBe(true);
      expect(isProtocolMessage("[PROTOCOL:STATUS_UPDATE] from x at y")).toBe(true);
    });
