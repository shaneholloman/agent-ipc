...
export class SessionLogger {
  ...
  constructor(sessionName?: string, logsDir?: string) {
    ...
    const providedName = sessionName || process.env.TMUX_SESSION || "unknown-session";
    ...
    if (sessionState?.descriptor) {
      ...
      logger.debug({ descriptor: this.descriptor }, "Using persistent descriptor from session state");
    } else {
      ...
      logger.debug({ descriptor: this.descriptor }, "Generated new descriptor (no session state)");
    }
   ...
   * Log a custom entry
   */
  logCustom(name: string, data: Record<string, unknown>, target?: string): void {
    const entry = this.createEntry(
      "custom",
      {
        name,
